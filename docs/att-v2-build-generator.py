...
function ukTakeHome(salary,pension=5,student=0){salary=Math.max(0,salary); const pensionAmt=salary*pension/100; const taxable=Math.max(0,salary-pensionAmt-12570); const incomeTax=Math.min(taxable,37700)*.2+Math.min(Math.max(0,taxable-37700),87440)*.4+Math.max(0,taxable-125140)*.45; const niPay=Math.max(0,salary-12570); const ni=Math.min(niPay,37700)*.08+Math.max(0,niPay-37700)*.02; const studentRepay=Math.max(0,salary-student)*.09; const net=Math.max(0,salary-incomeTax-ni-pensionAmt-studentRepay); return {net,tax:incomeTax,ni,pension:pensionAmt,student:studentRepay,deductions:salary-net};}
function usTakeHome(salary,pension=0,state=4){salary=Math.max(0,salary); const retirement=salary*pension/100; const taxable=Math.max(0,salary-retirement-14600); const fed=Math.min(taxable,11600)*.10+Math.min(Math.max(0,taxable-11600),35550)*.12+Math.min(Math.max(0,taxable-47150),53400)*.22+Math.max(0,taxable-100550)*.24; const fica=salary*.0765; const st=salary*state/100; const net=Math.max(0,salary-fed-fica-st-retirement); return {net,tax:fed,ni:fica,pension:retirement,student:st,deductions:salary-net};}
function payCalc(form){const salary=num(form,'salary',50000), pension=num(form,'pension',5), region=(form.elements.region?.value||'UK'), state=num(form,'stateTax',4), student=num(form,'studentThreshold',27295); return region==='US'?usTakeHome(salary,pension,state):ukTakeHome(salary,pension,student);}
...
function run(form){const type=form.dataset.calcType||'generic', currency=(form.elements.currency?.value||'GBP'); let salary=num(form,'salary',50000), amount=num(form,'amount',10000), rate=num(form,'rate',5), years=num(form,'years',5), monthly=num(form,'monthly',250), days=num(form,'days',3), miles=num(form,'miles',20), price=num(form,'price',1.55), mpg=num(form,'mpg',45), pension=num(form,'pension',5), bonus=num(form,'bonus',5000), salary2=num(form,'salary2',60000);
 if(['take-home','income-tax','national-insurance','student-loan','pension','salary-sacrifice','pay-rise','raise','bonus-tax','payroll','fica'].some(k=>type.includes(k))){let a=payCalc(form); let b=payCalc(Object.assign(form,{elements:new Proxy(form.elements,{get(t,p){ if(p==='salary') return {value:salary2}; return t[p];}})})); let retained=type.includes('raise')||type.includes('pay-rise')?Math.max(0,b.net-a.net):Math.max(0,payCalc(Object.assign(form,{elements:new Proxy(form.elements,{get(t,p){if(p==='salary')return {value:salary+bonus}; return t[p];}})})).net-a.net); show(form,[['Annual take-home',money(a.net,currency)],['Monthly take-home',money(a.net/12,currency)],['Estimated deductions',money(a.deductions,currency)]], type.includes('raise')||type.includes('pay-rise')?`Estimated extra take-home from the change: ${money(retained,currency)} per year.`:'This is an estimate using the assumptions shown on the page, not a payslip replacement.'); return; }
...
$$('form[data-calc-type]').forEach(form=>{form.addEventListener('submit',e=>{e.preventDefault();run(form)}); const b=form.querySelector('button'); if(b) b.addEventListener('click',()=>{});});
})();
'''

def calc_type(url, title):
    s=(url+' '+title).lower()
    for key in ['take-home','income-tax','national-insurance','student-loan','bonus-tax','overtime','pension','salary-sacrifice','pay-rise','raise','mortgage','loan','apr','debt','rent','household-budget','cost-of-living','childcare','council-tax','energy','car','fuel','inflation','saving','emergency','compound','regular-savings','isa','redundancy','holiday','sick','total-compensation','benefits','job-offer','salary-change','commute']:
...
def calc_form(row):
    ...
    region = '<label>Region<select name="region"><option>UK</option><option>US</option></select></label>'
    if any(k in t for k in ['mortgage','loan','apr']):
    ...
    return f'<form class="calc-panel" data-calc-type="{html.escape(t)}"><div class="input-grid">{inputs}</div><button type="submit">Calculate estimate</button><div class="result-panel" data-result hidden></div></form>'
