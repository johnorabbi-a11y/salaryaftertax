...
function run(form){const type=form.dataset.calcType||'generic', currency=(form.elements.currency?.value||'GBP'); let salary=num(form,'salary',50000), amount=num(form,'amount',10000), rate=num(form,'rate',5), years=num(form,'years',5), monthly=num(form,'monthly',250), days=num(form,'days',3), miles=num(form,'miles',20), price=num(form,'price',1.55), mpg=num(form,'mpg',45), pension=num(form,'pension',5), bonus=num(form,'bonus',5000), salary2=num(form,'salary2',60000);
 if(['take-home','income-tax','national-insurance','student-loan','pension','salary-sacrifice','pay-rise','raise','bonus-tax','payroll','fica','generic','holiday','sick','redundancy','overtime'].some(k=>type.includes(k))){let a=payCalc(form); let b=payCalc(form,salary2); let withBonus=payCalc(form,salary+bonus); let retained=type.includes('raise')||type.includes('pay-rise')||type.includes('job-change')?Math.max(0,b.net-a.net):Math.max(0,withBonus.net-a.net);
  ...
  if(type.includes('student-loan')){let threshold=num(form,'amount',27295), repayment=Math.max(0,salary-threshold)*rate/100; show(form,[['Annual repayment',money(repayment,currency)],['Monthly repayment',money(repayment/12,currency)],['Threshold used',money(threshold,currency)]],'Student-loan deductions are threshold-based estimates; plan and jurisdiction rules should be checked before relying on the figure.'); return;}
  ...
  if(type.includes('holiday')||type.includes('sick')||type.includes('redundancy')){let daily=salary/260, weekly=salary/52; show(form,[['Daily pay basis',money(daily,currency)],['Weekly pay basis',money(weekly,currency)],['Monthly take-home estimate',money(a.net/12,currency)]],'Employment-pay outputs use simple salary conversion assumptions and should be checked against your contract and statutory rules.'); return;}
